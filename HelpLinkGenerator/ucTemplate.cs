...
namespace HelpLinkGenerator {
    public partial class ucTemplate : UserControl {
        ...
        object GetDefaultData(int rowIndex, string propertyName) {
            string fullTemplateName = memberNames[rowIndex];
            string pattern = @"\d+$";
            string replacement = "";
            Regex rgx = new Regex(pattern);
            string TemplateNoMD = fullTemplateName.Remove(fullTemplateName.Length - 3, 3); //remove .md extension
            string TemplateNoMDID = rgx.Replace(TemplateNoMD, replacement); //remove numeric ID
            string TemplateID = TemplateNoMD.Replace(TemplateNoMDID, "");

            switch (propertyName) {
                case "Template Name":
                    return TemplateNoMDID; 
                case "ID":
                    return TemplateID;
                default:
                    return String.Empty;
        ...
        private void gridView1_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e) {
            ...
            string TemplateName = view.GetFocusedRowCellDisplayText("Template Name");
            string ID = view.GetFocusedRowCellDisplayText("ID");
            // [!include[RichEdit_Examples_Formatting_Change Formatting of Selected Text](~/templates\richedit_examples_formatting_change-formatting-of-selected-text111754.md)]
            buttonEdit1.EditValue = "[!include[" + TemplateName + @"](~/templates\" + TemplateName + ID + ".md)]";
        }
